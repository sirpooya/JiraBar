...
/// not really have.
///
/// It was reported as "pasting an image only works when the comment box is empty", which is the
...
/// Shortcuts that belong to the comment box rather than to text editing in general.
///
/// Matched by virtual key code for the same reason as `EditingShortcut`: the character a key
/// reports moves with the active input source, and this box is where Persian gets typed.
enum ComposerShortcut {
    ...
    /// Cmd+Return sends the comment, the way it does in Jira's own editor and most comment boxes.
    ///
    /// Return on its own stays a newline and must never send: a comment here is regularly several
    /// lines, and the composer grows to five of them precisely because that is normal.
